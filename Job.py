...
# Do i need a custom thread and a custom list of threads?
# now my program is running how can i change the status of the thread from inside the thread?
# i could get_thread from its id and then change its status but that doesnt sound too thread safe
# maybe i don't want a custom thread but an object with a thread attribute
class MyJob():
    def __init__(self, job_id: int, user: str, playlist_id: str, project_name: str, debug: bool = False,
        ...
        self.thread = threading.Thread(target=self.big_function, args=())
        self.thread.start()
